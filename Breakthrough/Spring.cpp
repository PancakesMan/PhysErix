...
void Spring::fixedUpdate(glm::vec2 gravity, float timeStep)
{
	...
	if (m_body1->isKinematic() && !m_body2->isKinematic()) {
		m_body2->applyForce(force * 2.f * timeStep, p2 - m_body2->getPosition());
		//m_body1->applyForce(-force * timeStep, p1 - m_body1->getPosition());
	}
	else if (m_body2->isKinematic() && !m_body1->isKinematic())
		m_body1->applyForce(-force* 2.f * timeStep, p1 - m_body1->getPosition());
		//m_body2->applyForce(force * timeStep, p2 - m_body2->getPosition());
	else if (!m_body1->isKinematic() && !m_body2->isKinematic())
	{
		m_body1->applyForce(-force * timeStep, p1 - m_body1->getPosition());
		m_body2->applyForce(force * timeStep, p2 - m_body2->getPosition());
	}
